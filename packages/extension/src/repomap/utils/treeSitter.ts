...
export async function getParserForFile(extensionPath: string, filepath: string) {
    try {
        await Parser.init();
        const parser = new Parser();

        const language = await getLanguageForFile(extensionPath, filepath);
        if (!language) {
            return undefined;
        }
...
export async function getLanguageForFile(
    ...
    try {
        ...
        if (!language) {
            language = await loadLanguageForFileExt(extensionPath, extension);
...
export async function getQueryForFile(
    ...
    const sourcePath = path.join(__dirname, "tree-sitter", queryPath);
...
export async function getSymbolsForFile(
    ...
    const parser = await getParserForFile(extensionPath, filepath);
...
export async function getSymbolsForManyFiles(
    ...
    const filesAndSymbols = await Promise.all(
        uris.map(async (uri): Promise<[string, SymbolWithRange[]]> => {
            ...
            const symbols = await getSymbolsForFile(extensionPath, uri, contents);
            return [uri, symbols ?? []];
        }),
    );
