...
import { getIndexFolderPath } from "../utils/paths";
...
import { RepoMapIndex } from "./RepoMapIndex";
import path from "path";
import { _RepoMap_ } from "../_repoMap_/_RepoMap_";
...
export class CodebaseIndexer {
  ...
  async clearIndexes() {
    ...
    //console.log(`Clearing indexes at ${sqliteFilepath} and ${lanceDbFolder}`);
    ...
    // }
    const indexFloder = getIndexFolderPath();
    console.log(`Clearing indexes at ${indexFloder}`);
    try {
      await fs.rm(indexFloder, { recursive: true, force: true });
    } catch (error) {
      console.error(`Error deleting ${indexFloder}: ${error}`);
    }
  }
  ...
  protected async getIndexesToBuild(): Promise<CodebaseIndex[]> {
    ...
    const indexes = [
      (await RepoMapIndex.getInstance()), // repoMap must come before chunk
    ];
  ...
  async *refreshDirs(
    ...
    for (const directory of dirs) {
      ...
      const directoryFiles: string[] = new Array<string>();
      for await (const p of walkDirAsync(directory, this.ide)) {
        // exclude .repomap files
        if (p.startsWith(path.join((await this.ide.getWorkspaceDirs())[0], ".repomap"))) {
          continue;
        }
